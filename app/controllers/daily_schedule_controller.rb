	...
	def new
		begin
			...
			if message != ""
				...
				if !c_date[0].nil?
					response = {origin: "", message: "Horario já reservado" , status: 400}
				else
					###  Horario livre, verificar se existe a sala
					room_l = MeetingRoom.where(id: params['room'].to_i).select(:id, :name, :local)
					if room_l[0].nil?
						response = {origin: "", message: "Sala não cadastrada" , status: 400}
					else
						###  Passou na validação, efetuar o agendamento
						room = room_l[0]
#						DailySchedule.room[0].create!(user_id: @current_user, subject: @params[:subject],start_date: @params[:start_time], end_date: @params[:end_time])
